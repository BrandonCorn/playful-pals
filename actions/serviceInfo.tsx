...
import { insertNewServiceInfo, updateServiceInfo } from '@/lib/db/serviceInfo';
...
const updateServiceInfoSchema = z.object({
  departureDate: z.string({
    required_error: 'Departure date is required',
    invalid_type_error: 'Departure date must be a string'
  }),
...
export async function updatePetServiceInfo(
  data: { serviceInfoId: string; path: string },
  state: any,
  formData: FormData
) {
  const { serviceInfoId, path } = data;
  const result = updateServiceInfoSchema.safeParse({
    departureDate: formData.get('departureDate'),
    departureTime: formData.get('departureTime'),
    breakfast: formData.get('breakfast'),
    lunch: formData.get('lunch'),
  ...
  if (result.error) {
    ...
    try {
      const updatedServiceInfo = await updateServiceInfo(
        serviceInfoId,
        newServiceInfo
      );
      if (!Array.isArray(updatedServiceInfo)) {
        return [];
      } else {
        revalidatePath(path, 'page');
      ...
      return { error: 'Error updating service info' };
    }
  }
}
